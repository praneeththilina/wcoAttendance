...
## 2024-03-24 - Prisma Distinct Query
**Learning:** Fetching many rows to map them to unique user statuses locally consumes memory and time.
**Action:** Use `distinct` and `orderBy` in Prisma to fetch only the latest row per user directly from the database instead of manually deduping on the JS side.
