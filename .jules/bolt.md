...
## 2024-05-10 - Prisma Distinct Operator for Deduplication
**Learning:** Using Prisma `findMany` to fetch large numbers of records (e.g., getting the latest status for all users) and deduplicating them in memory via loops and maps in Node.js creates unnecessary memory overhead and processing time.
**Action:** Use Prisma's `distinct` operator combined with `orderBy` to push the deduplication directly to the database, ensuring only the necessary records are fetched and returned, which optimizes performance.
