...
## 2026-04-30 - Optimize getAllStaff query by pushing join to Prisma
**Learning:** Sequential DB queries followed by JS-side loops (like `forEach` setting a Map) can be highly inefficient and scale linearly with user counts.
**Action:** Use Prisma's nested `select` or `include` (e.g. `attendance: { where: {...}, take: 1 }`) to let the database handle the relation mapping and return precisely the required data, removing manual Map constructions and O(N) operations in JS.
