...
## 2025-02-15 - Optimize Database Seeding
**Learning:** Sequential awaited loops over database operations scale poorly and act as an N+1 query bottleneck, especially observable during initial setups or seeding.
**Action:** I will group independent Prisma operations and resolve them with `Promise.all()` to decrease overall execution latency from O(N * latency) to O(latency).
