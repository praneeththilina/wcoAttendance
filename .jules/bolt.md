...
## 2024-04-18 - Avoid Barrel Files for Code Splitting
**Learning:** When using React.lazy() to implement code splitting, importing from barrel files (e.g., `import { Page } from "@/pages"`) will cause all modules exported by the barrel to be bundled together, defeating the purpose.
**Action:** Always import components directly from their specific files (e.g., `import("@/pages/Page")`) to ensure effective bundle separation.
