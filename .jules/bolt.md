...
## 2024-04-12 - Consolidating O(N) Array Operations
**Learning:** Computing multiple metrics (like active and inactive counts) using multiple `.filter()` passes causes unnecessary O(kN) iteration and CPU cycles.
**Action:** Use a single `useMemo` block with a one-pass loop (or reduce) to calculate all needed metrics in a single O(N) pass, saving iterations.
