...
## 2024-04-21 - Parallelize Independent Database Queries
**Learning:** Sequential Prisma queries within a controller, such as `.count()` and `.findMany()`, sum up latency which slows down dashboard initial loads.
**Action:** Always identify independent database calls and wrap them in a `Promise.all()` to fetch data in parallel and reduce overall request latency.
