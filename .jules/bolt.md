...
## 2024-04-20 - Parallelize sequential independent database queries
**Learning:** Sequential await statements for independent Prisma queries (like `findMany` and `count` on different tables) cause unnecessary cumulative latency, as the second query cannot start until the first completes.
**Action:** Identify independent database operations in controllers and wrap them in `await Promise.all([query1, query2])` to execute them concurrently, reducing total execution time to the duration of the slowest query rather than the sum of both.
