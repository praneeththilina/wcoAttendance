...
## 2026-05-19 - Database Aggregation for Deduplication
**Learning:** In the Prisma backend, retrieving all records to manually extract the latest instance per relational entity (e.g. latest attendance record per user) creates unnecessary memory bloat and O(N) iteration overhead.
**Action:** Use Prisma's `distinct` operator combined with `orderBy` to perform this deduplication efficiently directly inside the PostgreSQL database layer, offloading memory operations from the Node.js process. Note that for PostgreSQL compatibility, the fields in the `distinct` array must appear as the left-most fields in the `orderBy` array.
