...
## 2024-03-24 - Parallelize Database Queries
**Learning:** Sequential Prisma queries in highly used backend controllers (like `getDashboardStats` and `getAllStaff`) increase response times unnecessarily when the queries are independent.
**Action:** Always refactor sequential, independent `await prisma...` queries using `Promise.all()` to fetch data concurrently and reduce latency.
