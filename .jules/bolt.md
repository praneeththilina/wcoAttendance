...
## 2026-04-24 - Parallelizing Prisma Queries
**Learning:** In the Express/Prisma backend, independent sequential database queries (like a `.count()` followed by a `.findMany()`) create unnecessary sequential network roundtrips to the database.
**Action:** Always identify independent read queries in controllers and wrap them in `Promise.all()` to execute them concurrently, significantly reducing overall request latency.
