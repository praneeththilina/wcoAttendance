...
## 2024-03-24 - Array Multiple Metric Optimization
**Learning:** Using multiple `.filter().length` calls inside a React component to calculate related derived metrics (like active vs inactive counts) results in O(k * N) complexity that runs on every render.
**Action:** Consolidate multiple filter passes over the same array into a single O(N) loop within a `useMemo` block, especially for arrays that can grow large or in components that re-render frequently (e.g. during typing).
