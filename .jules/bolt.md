...
## 2024-03-24 - Temporary Scripts Cleanup
**Learning:** Forgetting to delete temporary file-patching scripts (like Python regex replacement scripts) pollutes the repository and causes PR review failures.
**Action:** Always run a cleanup command (like `rm patch_*.py`) immediately after using ad-hoc tooling scripts to ensure the Git tree remains clean for commits.
