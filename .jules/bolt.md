...
## 2026-05-08 - Prisma Distinct and Memory Usage
**Learning:** In Prisma queries that involve selecting multiple rows just to group or deduplicate them manually in JavaScript code (which leads to N+1-like memory scaling issues), it's significantly better to push the deduplication logic to the database layer by using `distinct: ['fieldName']` paired with `orderBy`.
**Action:** Use `distinct` along with `orderBy` whenever fetching the "latest" or "first" record for related groups instead of returning all rows and manually filtering them in Node.
