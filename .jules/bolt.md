...
## 2024-04-22 - Parallelizing Database Queries
**Learning:** Sequential, independent database queries inside a single API controller route block block overall processing.
**Action:** Identify independent database operations (e.g. counting total rows versus fetching a list of recent rows) and wrap them in `Promise.all()` to execute them concurrently, reducing total network latency.
