...
## 2026-03-12 - Prisma distinct pushdown
**Learning:** You can push down deduplication of related data to the database using Prisma's `distinct` operator along with `orderBy` instead of manually filtering the array in JavaScript using a Map or loop.
**Action:** Use Prisma's `distinct` for fetching the latest unique related record to save Node.js memory overhead.
