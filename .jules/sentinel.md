...
## 2024-05-18 - Mass Assignment in Controllers
**Vulnerability:** Admin controller methods directly destructured `req.body` to update staff user records without proper validation via schemas, allowing a potential mass assignment privilege escalation.
**Learning:** Even if some properties like password are removed explicitly before updating, the lack of full request validation using Zod exposes the system to data tampering. Blocklists alone can fail if new fields are added later.
**Prevention:** Always parse and extract variables from Zod schema-validated objects (e.g. `schema.parse(req.body)`) instead of `req.body` directly, combining validation with defense-in-depth blocklists.
