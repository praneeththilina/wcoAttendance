...
## 2024-04-03 - [Fix mass assignment vulnerability in changeLocation]
**Vulnerability:** The `changeLocation` endpoint extracted properties directly from `req.body` without strict validation schemas, exposing a potential mass assignment vulnerability or allowing malformed payloads to reach internal functions.
**Learning:** Inconsistent application of Zod schemas in controllers. While `checkIn`, `checkOut`, and `getHistory` were protected via `parse`, `changeLocation` was left without schema validation.
**Prevention:** Every API endpoint reading properties from request payloads (body, query, params) must have its inputs fully validated and sanitized using comprehensive Zod schemas before proceeding to the service layer.
