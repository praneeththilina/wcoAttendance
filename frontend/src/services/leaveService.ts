...
export const leaveService = {
  ...
  async getMyRequests(
    ...
    limit = 20,
    status?: string
  ): Promise<{ records: LeaveRequest[]; total: number }> {
  ...
  async updateRequestStatus(
    ...
    const response = await apiClient.put<{ success: boolean; data: LeaveRequest }>(
      `/leaves/request/${requestId}/status`,
      { status }
    );
    return response.data.data;
  },
  ...
  async updateLeaveBalance(
    ...
    const response = await apiClient.put<{ success: boolean; data: LeaveBalance }>(
