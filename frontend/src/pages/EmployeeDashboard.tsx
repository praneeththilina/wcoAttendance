...
export function EmployeeDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 antialiased">
      <div className={`max-w-md mx-auto border-x ${isVariant1 ? 'border-primary/10' : 'border-transparent'}`}>
        {/* Header */}
        <header className={`flex items-center ${isVariant1 ? 'bg-background-light dark:bg-background-dark' : 'bg-white dark:bg-background-dark'} p-4 ${isVariant1 ? 'pb-2' : 'pb-4'} justify-between sticky top-0 z-10 border-b ${isVariant1 ? 'border-primary/5' : 'border-slate-200 dark:border-slate-800'}`}>
          <div className="flex size-10 shrink-0 items-center">
            <div className={`flex items-center justify-center rounded-full size-10 ${isVariant1 ? 'bg-primary/10' : 'bg-primary/5 border border-primary/10'}`}>
              <span className="material-symbols-outlined text-primary">account_circle</span>
            </div>
          </div>
          <h2 className="text-lg font-bold leading-tight tracking-[-0.015em] flex-1 ml-3">Attendance</h2>
          
          {(!isOnline || isSyncing) && (
            <div className={`mr-2 flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
              isSyncing ? 'bg-blue-100 text-blue-700 animate-pulse' : 'bg-amber-100 text-amber-700'
            }`}>
              <span className="material-symbols-outlined text-sm">{isSyncing ? 'sync' : 'cloud_off'}</span>
        ...
        <div className={`flex ${isVariant1 ? 'p-4 @container' : 'px-6 py-8 @container bg-white dark:bg-background-dark/30'} ${!isVariant1 ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}>
          <div className="flex w-full flex-col gap-4 items-start">
            ...
            <StatusBadge status={isCheckedIn ? 'checked_in' : isCheckedOut ? 'checked_out' : 'not_checked_in'} />
          </div>
        </div>
        ...
        <div className="px-4 py-2">
          <div className={`bg-white dark:bg-background-dark/50 rounded-xl border p-5 shadow-sm ${
            isVariant1 
              ? 'border-primary/5' 
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Arrival Time</span>
                </div>
                <span className="text-slate-900 dark:text-slate-100 font-semibold">
                  {status?.checkInTime ? new Date(status.checkInTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '---'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Current Client</span>
                </div>
        ...
        <div className="px-4 py-4 mt-2">
          <div className={`relative w-full overflow-hidden border bg-primary/5 flex items-center justify-center ${
            isVariant1 
            ...
            <div className="relative bg-white/90 dark:bg-background-dark/90 px-3 py-1.5 rounded-full flex items-center shadow-md">
              <span className="material-symbols-outlined text-primary mr-1 text-sm">my_location</span>
