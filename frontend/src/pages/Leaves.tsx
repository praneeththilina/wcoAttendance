              ...
              <div className="flex gap-4 w-max">
                <div className="bg-primary text-white w-40 p-4 rounded-2xl shadow-md snap-start shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    <span className="material-symbols-outlined bg-white/20 p-2 rounded-xl">medical_services</span>
                  </div>
                  <span className="block text-sm font-medium text-white/80 mb-1">Sick Leave</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-bold">{balance ? balance.sickLeaveTotal - balance.sickLeaveUsed : 0}</span>
                    <span className="text-sm font-medium text-white/60">/ {balance?.sickLeaveTotal || 7} days</span>
                  </div>
                </div>

                <div className="bg-[#1e293b] text-white w-40 p-4 rounded-2xl shadow-md snap-start shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    <span className="material-symbols-outlined bg-white/10 p-2 rounded-xl">flight_takeoff</span>
                  </div>
                  <span className="block text-sm font-medium text-slate-300 mb-1">Annual Leave</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-bold">{balance ? balance.annualLeaveTotal - balance.annualLeaveUsed : 0}</span>
                    <span className="text-sm font-medium text-slate-400">/ {balance?.annualLeaveTotal || 15} days</span>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 w-40 p-4 rounded-2xl shadow-sm snap-start shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    <span className="material-symbols-outlined bg-slate-100 dark:bg-slate-700 text-slate-500 p-2 rounded-xl">calendar_month</span>
                  </div>
                  <span className="block text-sm font-medium text-slate-500 mb-1">Unpaid Leave</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-bold text-slate-800 dark:text-white">{balance?.unpaidLeaveUsed || 0}</span>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
              ...
              <form onSubmit={handleRequestSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Total Days</label>
                    <input 
                      type="number"
                      min="0.5"
                      step="0.5"
              ...
              {requests.length === 0 ? (
                ...
                <div className="space-y-3">
                  {requests.map(request => (
                    <div key={request.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex items-center justify-between group hover:border-primary/20 transition-all">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${
                          request.type === 'sick' ? 'bg-red-50 text-red-500 dark:bg-red-500/10' :
                          request.type === 'annual' ? 'bg-blue-50 text-blue-500 dark:bg-blue-500/10' :
                          'bg-slate-50 text-slate-500 dark:bg-slate-500/10'
                        }`}>
                          <span className="material-symbols-outlined">
                            {request.type === 'sick' ? 'medical_services' : request.type === 'annual' ? 'flight_takeoff' : 'event_note'}
                          </span>
                        </div>
                        <div>
                          ...
                          {request.reason && <p className="text-[10px] text-slate-400 mt-1 truncate max-w-[150px]">{request.reason}</p>}
                        </div>
                      </div>
