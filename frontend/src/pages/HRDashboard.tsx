  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 pb-20">
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
        <h2 className="text-xl font-bold mb-4">Firm Overview</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h2 className="text-3xl font-bold text-primary">{isLoading ? '-' : stats?.totalEmployees || 0}</h2>
            </div>
            <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
              <span className="material-symbols-outlined max-text-3xl">badge</span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h2 className="text-3xl font-bold text-success">{isLoading ? '-' : stats?.activeToday || 0}</h2>
            </div>
            <div className="h-12 w-12 rounded-full bg-success/10 flex items-center justify-center text-success">
              <span className="material-symbols-outlined max-text-3xl">how_to_reg</span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h2 className="text-3xl font-bold text-primary">{isLoading ? '-' : `${(stats?.complianceRate || 0).toFixed(1)}%`}</h2>
            </div>
            <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
              <span className="material-symbols-outlined max-text-3xl">fact_check</span>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-slate-500">No attendance records for today.</td>
                  </tr>
                ) : (
                  stats.todaysAttendance.map((record) => (
                    <tr key={record.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-100">
                        {record.user.firstName} {record.user.lastName} <span className="text-xs text-slate-500 block">{record.user.employeeId}</span>
                      </td>
                      <td className="px-6 py-4">
                        {record.client?.name || 'Unknown'} <span className="text-xs text-slate-500 block">{record.client?.city || ''}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${
                          record.status === 'checked_in' ? 'bg-success/10 text-success' : 
                          record.status === 'checked_out' ? 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300' : 
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm mt-8">
          <div className="p-6">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <button className="flex items-center gap-3 p-4 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-primary hover:bg-primary/5 transition-all text-left group">
                 <div className="bg-primary/10 p-3 rounded-full text-primary group-hover:scale-110 transition-transform">
                   <span className="material-symbols-outlined">download</span>
                 </div>
                 <div>
                   <h4 className="font-bold">Export Payroll Data</h4>
                   <p className="text-xs text-slate-500">Download CSV for the current period</p>
                 </div>
               </button>
               <button className="flex items-center gap-3 p-4 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-primary hover:bg-primary/5 transition-all text-left group">
                 <div className="bg-primary/10 p-3 rounded-full text-primary group-hover:scale-110 transition-transform">
                 ...
                 <div>
                   <h4 className="font-bold">Audit Trail Logs</h4>
                   <p className="text-xs text-slate-500">View detailed system access logs</p>
                 </div>
               </button>
               <button 
               ...
               >
                 ...
                 <div>
                   ...
                   <p className="text-xs text-slate-500">Review and approve employee leaves</p>
                 </div>
               </button>
             </div>
          </div>
