...
// ⚡ Bolt: Extracted staff list item into a React.memo component.
// Impact: Prevents O(N) re-renders of list items when typing in the search bar or polling data.
const StaffListItem = memo(({ member }: { member: LocalStaffMember }) => (
        ...
        <div className="relative flex min-h-screen w-full flex-col w-full max-w-7xl mx-auto md:px-8 lg:px-12 bg-white dark:bg-background-dark shadow-2xl overflow-x-hidden">
          ...
          <div className="flex flex-col px-4 pb-24 gap-3">
            {isLoading ? (
              ...
              filteredStaff.map((member) => <StaffListItem key={member.id} member={member} />)
            )}
          </div>
        </div>
