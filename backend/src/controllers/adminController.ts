...
export const adminController = {
  ...
  updateStaff: async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        // Parse and validate the input data to prevent mass assignment vulnerabilities
        const validatedData = updateStaffSchema.shape.body.parse(req.body);
        const { email, firstName, lastName, role, isActive } = validatedData;
