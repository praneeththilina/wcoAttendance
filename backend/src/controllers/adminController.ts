...
export const adminController = {
  ...
  createStaff: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const validatedData = createStaffSchema.shape.body.parse(req.body);
      const { employeeId, email, password, firstName, lastName, role, isActive } = validatedData;
  ...
  updateStaff: async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        const updateData = updateStaffSchema.shape.body.parse(req.body) as any;
