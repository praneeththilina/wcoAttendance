...
export const adminController = {
  ...
  getAllStaff: async (_req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      // ⚡ Bolt: Pushed deduplication down to the database using `distinct` combined with `orderBy`.
      // This prevents fetching unnecessary historical records and reduces O(N) memory overhead in Node.js.
      const records = await prisma.attendanceRecord.findMany({
