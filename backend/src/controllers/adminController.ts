...
export const adminController = {
  ...
  createStaff: async (req: Request, res: Response, next: NextFunction) => {
    try {
      // Validate to prevent mass assignment
      const { employeeId, email, password, firstName, lastName, role, isActive } =
        createStaffSchema.shape.body.parse(req.body);
  ...
  updateStaff: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      // Parse and validate update data to prevent mass assignment vulnerabilities
