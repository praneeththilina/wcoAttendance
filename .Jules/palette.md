...
## 2026-04-10 - Added ARIA labels and semantic buttons to Admin Dashboard
**Learning:** Icon-only controls in the header (menu and notifications) were using non-semantic `<div>` elements or lacked `aria-label` attributes and keyboard focus indicators, making them inaccessible to screen readers and keyboard users.
**Action:** Always prefer semantic HTML tags like `<button>` for interactive elements over `<div>`. For icon-only controls, always provide an `aria-label` to describe the action, and include focus ring styles (`focus-visible:ring-2 focus-visible:ring-primary`) to ensure keyboard navigability.
