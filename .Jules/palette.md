...
## 2025-04-08 - Added aria-labels to multiple dashboard icon buttons
**Learning:** Many administrative dashboards (like HRDashboard, DailyAttendanceReport, ManagerDashboard) were using pure icon-only `<button>` tags without any `aria-label` or inner text. This made elements like "Notifications", "Help", and "Action Menus" entirely opaque to screen readers despite their critical functions.
**Action:** Consistently audit all new navigation and dashboard headers for icon-only buttons (`span` with material-symbols inside a `button`) and enforce descriptive `aria-label`s reflecting the action (e.g., "Open menu", "Notifications", "Logout").
