...
## 2025-02-14 - Accessible Dashboard Action Buttons
**Learning:** Found critical action buttons in the Admin Dashboard (e.g., 'Notifications' icon and 'View All' text) missing keyboard focus indicators (`focus-visible`). The icon button also lacked semantic context (`aria-label`). Without focus rings, keyboard users cannot navigate the dashboard interactively. The project requires the explicit standard tailwind combination of `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary`.
**Action:** Always verify that interactive icon buttons have `aria-label` attributes and explicit `focus-visible` utility classes. For generic text buttons not using the shared `<Button>` component, manually inject the standard focus ring classes along with rounded corners to ensure visibility.
