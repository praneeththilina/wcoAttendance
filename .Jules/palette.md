...
## 2025-03-29 - Missing aria-labels in Top App Bars
**Learning:** Found critical header action buttons (menu toggle, notifications) in `AdminDashboard.tsx` that were implemented either as a `div` or a `<button>` without an `aria-label`. Since these are icon-only buttons (`material-symbols-outlined`), they are completely opaque to screen readers, and the use of `div` prevents standard keyboard focus.
**Action:** When working on top app bars or navigation headers, always verify that all icon-only interactive elements are `<button type="button">` tags (not `div`s) and have descriptive `aria-label` attributes (e.g., "Toggle menu", "Notifications"). Ensure `focus-visible` styles are included to support keyboard navigation.
