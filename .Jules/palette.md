...
## 2024-04-28 - Custom Toggle Switch Accessibility
**Learning:** In the Settings page, custom styled buttons were being used as toggle switches (e.g., for Dark Mode, Notifications) without any semantic meaning. Screen readers would just read them as "Button" without indicating their state or purpose.
**Action:** Always add `role="switch"`, an appropriate `aria-label`, and a dynamic `aria-checked={state}` attribute to any custom toggle switch component so assistive technologies can correctly identify its function and current state. Also ensure explicit `focus-visible` styles are added for keyboard navigation visibility.
