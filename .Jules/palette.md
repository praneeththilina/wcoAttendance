...
**Learning:** The frontend uses Google Material Symbols for icons within `<button>` tags. These icon-only buttons often lack `aria-label` attributes and visible focus states, creating severe accessibility barriers for screen reader and keyboard users.
**Action:** When implementing or modifying icon-only buttons, always ensure an `aria-label` is provided and include `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary` utility classes to maintain keyboard navigation clarity.
