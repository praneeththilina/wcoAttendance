...
## 2025-02-12 - Accessible Toggle Switches
**Learning:** Found custom toggle switches implemented as `<button>` elements in Settings without `role="switch"` or `aria-checked` attributes. This leaves screen readers unable to announce their state (on/off) or function, confusing users who rely on assistive technologies.
**Action:** When implementing custom toggle switches using buttons, always add `role="switch"`, dynamically update `aria-checked`, and provide descriptive `aria-label`s. Also, ensure standard keyboard focus visibility using classes like `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary`.
