...
## 2026-04-25 - Accessible Toggle Switches
**Learning:** Custom UI toggle switches (using standard `<button>` tags) frequently lack semantic `role="switch"` and `aria-checked` properties. Without these, screen readers announce them generically as "buttons" without conveying their boolean state.
**Action:** When auditing or building custom toggles (like in Settings screens), ensure you add `role="switch"`, `aria-checked`, `aria-labelledby`, and standard `focus-visible` keyboard styling.
