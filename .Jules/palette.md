...
## 2024-05-18 - [Add accessibility attributes to custom toggle switches]
**Learning:** Found custom toggle switches built with `<button>` elements that lacked ARIA switch attributes (`role="switch"`, `aria-checked`), making it impossible for screen reader users to know their state or purpose. Furthermore, they lacked `focus-visible` styles, hindering keyboard navigation.
**Action:** When implementing custom toggle switches, always include `role="switch"` and `aria-checked={state}`. Add appropriate `aria-label`s for context, and ensure standard focus styles (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) are present for keyboard accessibility.
