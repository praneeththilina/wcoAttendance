...
## 2024-04-01 - ARIA DescribedBy Implementation
**Learning:** We need to explicitly bind contextual helper and error messages to their inputs in the `Input` component using `aria-describedby`. Because error and helper text is passed generically into the component, screen readers don't natively know to associate that text string with the input field simply because it visually sits below it.
**Action:** Always generate unique HTML `id` properties for dynamic helper/error strings in shared form components and bind them via `aria-describedby` on the input element.
