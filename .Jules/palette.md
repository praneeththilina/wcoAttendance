...
## 2024-05-20 - [ARIA Labels and Focus States for Dashboard Icons]
**Learning:** Found several icon-only buttons (like menu, notifications, help, and more options) in `AdminDashboard.tsx` and `HRDashboard.tsx` lacking `aria-label`s and visual focus indicators (`focus-visible`). Additionally, some interactive icons were implemented as `<div>` elements instead of semantic `<button>` elements, breaking keyboard accessibility.
**Action:** When working on dashboards with icon-only controls, always ensure semantic `<button>` tags are used. Add descriptive `aria-label` and `title` attributes for screen readers and tooltips. Use Tailwind's `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary` utility classes to provide clear visual feedback for keyboard users.
