...
## 2025-03-12 - ARIA Labels for Icon-Only Action Buttons in Lists/Headers
**Learning:** Found multiple icon-only action buttons (e.g., `edit`, `delete`, `add`, `notifications`, `menu`) lacking `aria-label`s across various dashboard and list views (`AdminClients.tsx`, `AdminDashboard.tsx`, `HRDashboard.tsx`). This pattern is prevalent in headers and quick action lists. Also noticed a `div` element being styled like a menu button without semantic HTML or interactions, while other similar elements were `<button>` tags.
**Action:** When working on lists and headers, systematically verify all icon-only interactions. Convert non-semantic elements (`<div>`, `<span>`) to `<button>` where appropriate. Always attach specific, actionable `aria-label`s (e.g., "Edit client", "Open menu") rather than generic ones, keeping the context clear for screen reader users.
