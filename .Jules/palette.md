...
## 2025-05-13 - Truncation of command outputs in verification
**Learning:** When trying to read large sections of code to verify exact DOM structures (e.g. for ), commands like `cat` or `sed -n` can truncate output at 1000 characters. If the target element is near the bottom, it will be missed, leading to Groundedness Rule violations in plans.
**Action:** To prevent truncation when verifying large components, use `sed -n` with tight, specific line ranges (e.g., spanning 10-15 lines) exactly around the target element rather than attempting to print 50+ lines at once.
## 2025-05-13 - Truncation of command outputs in verification
**Learning:** When trying to read large sections of code to verify exact DOM structures, commands like `cat` or `sed -n` can truncate output at 1000 characters. If the target element is near the bottom, it will be missed, leading to Groundedness Rule violations in plans.
**Action:** To prevent truncation when verifying large components, use `sed -n` with tight, specific line ranges (e.g., spanning 10-15 lines) exactly around the target element rather than attempting to print 50+ lines at once.
