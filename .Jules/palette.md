...
## 2024-10-27 - Accessible Toggle Switches
**Learning:** Found custom toggle switches implemented as `<button>` elements but lacking semantic meaning. Without proper ARIA attributes, screen readers interpret them as regular buttons without indicating their current state (on/off).
**Action:** When implementing custom toggle switches, always add `role="switch"` and dynamically update `aria-checked={state}`. Additionally, provide descriptive `aria-label`s and ensure robust keyboard navigation support by adding proper focus ring styles (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary`).
